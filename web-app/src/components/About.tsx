    ...
    return (
        <section id="about" style={{ padding: '100px 0' }}>
            <div className="container">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 0.8fr', gap: '5rem', alignItems: 'center' }}>
                    <motion.div
                        ...
                        <ul style={{ padding: 0 }}>
                            {['Expert Custom Development', 'Scalable Cloud Foundations', 'Data-Driven Business Automation', '24/7 Technical Support'].map((item, i) => (
                                <li key={i} style={{ position: 'relative', paddingLeft: '30px', marginBottom: '0.8rem', fontWeight: 500, listStyle: 'none' }}>
                                    <span style={{ position: 'absolute', left: 0, color: 'var(--secondary)', fontWeight: 900 }}>✓</span>
                                    {item}
                                </li>
                            ))}
                        </ul>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }}>
                        {[
                            ...
                            <motion.div 
                                ...
                                style={{ padding: '2.5rem 1.5rem', background: 'var(--light)', borderRadius: '16px', textAlign: 'center', border: '1px solid rgba(10, 37, 64, 0.05)' }}
                            >
                                <h3 style={{ fontSize: '2.5rem', color: 'var(--secondary)', marginBottom: '0.5rem' }}>{stat.val}</h3>
                                <p style={{ fontSize: '0.9rem', color: 'var(--primary)', fontWeight: 600 }}>{stat.label}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
